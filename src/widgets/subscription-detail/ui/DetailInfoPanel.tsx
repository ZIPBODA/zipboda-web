  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <span className={`rounded-md px-2.5 py-1 text-xs font-semibold ${AGENCY_TAG_TONE[detail.agency]}`}>
          {detail.agencyLabel}
        </span>
        <span className="rounded-md bg-brand/50 px-2.5 py-1 text-xs font-bold text-fg-strong">
          D-{detail.dday} 마감 임박
        </span>
        <span className={`rounded-md px-2.5 py-1 text-xs font-semibold ${STATUS_BADGE_TONE[detail.status]}`}>
          {detail.status}
        </span>
      </div>
      ...
      <section className="py-5">
        ...
        <div className="mt-2.5 grid grid-cols-3 gap-2.5">
          {detail.units.map((unit) => {
            const selected = unit.size === selectedSize;
            return (
              <Link
                key={unit.size}
