    ...
    <Link href={`/subscriptions/${item.id}`} className="block overflow-hidden rounded-xl border border-line-subtle bg-surface">
      <div className="relative h-[120px] bg-surface-tertiary">
        ...
        <div className="absolute inset-x-0 top-0 flex gap-1 p-2.5">
          <span className={`rounded-sm px-1.5 py-0.5 text-caption font-bold text-fg-ondark ${AGENCY_BADGE_BG[item.agency]}`}>{item.agency}</span>
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 p-3">
          ...
          <span className={`flex size-11 shrink-0 items-center justify-center rounded-md text-sm font-bold text-fg-heading ${urgent ? "bg-brand" : "bg-surface"}`}>
            D-{item.dday}
          </span>
        </div>
      </div>
